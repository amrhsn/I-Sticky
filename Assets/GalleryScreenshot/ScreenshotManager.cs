...
public class ScreenshotManager : MonoBehaviour {
	...
	void Awake() 
	{
        ...
        sticky = FindObjectOfType<I_Stcky_Manager>();
	}
    ...
    public static void SaveScreenshot(string fileName, string albumName = "MyScreenshots", string fileType = "jpg", bool save = false, Rect screenArea = default(Rect))
	{
		...
		Instance.StartCoroutine(Instance.GrabScreenshot(save,fileName, albumName, fileType, screenArea));
	}
	
	IEnumerator GrabScreenshot(bool save,string fileName, string albumName, string fileType, Rect screenArea)
	{
		...
		texture.Apply ();
        sticky.ShowScreenShot(texture,save);
        ...
        if(save)Instance.StartCoroutine(Instance.Save(bytes, fileName, path, ImageType.SCREENSHOT));
	...
	IEnumerator Save(byte[] bytes, string fileName, string path, ImageType imageType)
	{
        ...
        Debug.Log("Saved ya m3lm , eb2a 2ablny");
